...
void Sprite::drawUsername(Vector pos, Game *game)
{
    ...
    if (pos.x - game->pos.x - (strlen(name) * 2 + 8) < 0 || pos.x - game->pos.x + (strlen(name) * 2 + 8) > game->size.x ||
    ...
    game->draw->fillRectangle(Vector(pos.x - game->pos.x - (strlen(name) * 2 - 5), pos.y - game->pos.y - 7), Vector(strlen(name) * 4 + 1, 8), COLOR_WHITE);

    // draw name over player's head
    game->draw->text(Vector(pos.x - game->pos.x - (strlen(name) * 2 - 4), pos.y - game->pos.y - 2), name, COLOR_BLACK);
}
...
void Sprite::update(Game *game)
{
    ...
    case ENTITY_IDLE:
        ...
        if (elapsed_move_timer >= move_timer)
        {
            ...
            if (fabs(position.x - start_position.x) < 1 && fabs(position.y - start_position.y) < 1)
            {
